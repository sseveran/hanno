...
class TestReindex:
    async def test_reindex_paginates_runs(self, search):
        runs = [
            _make_run(run_id=f"run-{i}", title=f"deploy server {i}")
            for i in range(1001)
        ]
        ...
        class PagedStorage:
            ...
            async def list_runs(
                self, *, limit: int = 50, offset: int = 0, **_: object
            ) -> list[Run]:
            ...
            async def list_step_runs(self, run_id: str) -> list[StepRun]:
                return []
        ...
        count = await search.reindex_all(storage)

        assert count == 1001
        assert storage.calls[:2] == [(1000, 0), (1000, 1000)]

        results = await search.search(
            "deploy",
            entity_types={EntityType.RUN},
            limit=2000,
        )
