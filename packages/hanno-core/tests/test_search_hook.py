...
class TestSearchIndexerHookIntegration:
    ...
    async def test_attach_artifact_indexes_content(self, search_ledger, actor):
        ledger, search = search_ledger
        run = await ledger.create_run("debug", actor=actor, title="Investigate error")
        await ledger.attach_artifact(
            run.id,
            data=b"stack trace: NullPointerException in worker thread",
            kind="log",
            actor=actor,
            name="error.log",
        )
        ...
        artifact_results = [r for r in results if r.entity_type == EntityType.ARTIFACT]
