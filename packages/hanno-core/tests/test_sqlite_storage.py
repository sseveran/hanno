...
class TestWorkspaceCRUD:
    ...
    async def test_find_by_external_ref(self, storage):
        workspace = Workspace(
            title="Alpha",
            external_refs=[{"system": "github", "ref_type": "org", "ref_id": "openai"}],
        )
        await storage.create_workspace(workspace)
...
class TestWorkspaceRepoCRUD:
    async def test_create_and_list(self, storage, sample_workspace):
        ...
        assert len(repos) == 1
        assert repos[0].canonical_remote == "github.com/openai/hanno"
...
class TestTaskCRUD:
    ...
    async def test_list_by_workspace_and_status(self, storage, sample_workspace):
        active = Task(workspace_id=sample_workspace.id, title="A")
        closed = Task(
            workspace_id=sample_workspace.id,
            title="B",
            status=TaskStatus.CLOSED,
        )
        await storage.create_task(active)
        await storage.create_task(closed)
    ...
    async def test_find_by_external_ref(self, storage, sample_workspace):
        task = Task(
            workspace_id=sample_workspace.id,
            title="PR work",
            external_refs=[{"system": "github", "ref_type": "pr", "ref_id": "org/repo#42"}],
        )
        await storage.create_task(task)
...
class TestTaskRepoLinks:
    async def test_create_and_list(self, storage, sample_task, sample_repo):
        link = TaskRepoLink(task_id=sample_task.id, workspace_repo_id=sample_repo.id)
        await storage.create_task_repo_link(link)
...
class TestRunCRUD:
    async def test_create_and_get(self, storage, sample_workspace):
        ...
        fetched = await storage.get_run(created.id)
        assert fetched is not None
        assert fetched.workspace_id == sample_workspace.id
