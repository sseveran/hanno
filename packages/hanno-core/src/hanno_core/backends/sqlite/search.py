...
class SqliteFtsSearchBackend:
    ...
    async def index_run(self, run: Run) -> None:
        await self._index_run(run)

    async def _index_run(self, run: Run, *, commit: bool = True) -> None:
    ...
    async def _index_event(self, event: Event, *, commit: bool = True) -> None:
        await self._db.execute(
    ...
    async def index_step_run(self, step: StepRun) -> None:
        await self._index_step_run(step)

    async def _index_step_run(self, step: StepRun, *, commit: bool = True) -> None:
        await self._db.execute(
    ...
    async def index_artifact(
        ...
        await self._index_artifact(artifact, content=content)
    ...
    async def reindex_all(
        ...
        offset = 0
        while True:
            runs = await storage.list_runs(limit=_REINDEX_BATCH_SIZE, offset=offset)
            if not runs:
                break

            for run in runs:
                await self._index_run(run, commit=False)
                count += 1

                events = await storage.list_events(run.id)
                for event in events:
                    await self._index_event(event, commit=False)
                    count += 1

                steps = await storage.list_step_runs(run.id)
                for step in steps:
                    await self._index_step_run(step, commit=False)
                    count += 1

                run_artifacts = await storage.list_artifacts(run.id)
                for artifact in run_artifacts:
                    content = None
                    if artifacts is not None:
                        with contextlib.suppress(Exception):
                            content = await artifacts.retrieve(artifact.uri)
